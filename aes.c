#include "aes.h"
#include <string.h>

#define Nb 4
#define Nk 8
#define Nr 14

static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

#define GETU32(pt) (((uint32_t)(pt)[0] << 24) ^ ((uint32_t)(pt)[1] << 16) ^ ((uint32_t)(pt)[2] << 8) ^ ((uint32_t)(pt)[3]))

static uint32_t SubWord(uint32_t w) {
    return ((uint32_t)sbox[(uint8_t)(w >> 24)] << 24) ^
           ((uint32_t)sbox[(uint8_t)(w >> 16)] << 16) ^
           ((uint32_t)sbox[(uint8_t)(w >> 8)] << 8) ^
           (uint32_t)sbox[(uint8_t)w];
}

static uint32_t RotWord(uint32_t w) {
    return (w << 8) | (w >> 24);
}

static uint32_t Rcon(uint8_t i) {
    uint32_t ret = 1;
    while (--i) ret = (ret << 1) ^ ((ret & 0x80) ? 0x1B : 0);
    return ret << 24;
}

void AES_init_ctx(struct AES_ctx* ctx, const uint8_t* key) {
    AES_init_ctx_iv(ctx, key, ctx->Iv);
}

void AES_init_ctx_iv(struct AES_ctx* ctx, const uint8_t* key, const uint8_t* iv) {
    uint32_t i, temp;
    uint32_t *rk = (uint32_t*)ctx->RoundKey;

    for (i = 0; i < Nk; i++) rk[i] = GETU32(&key[i * 4]);
    for (i = Nk; i < Nb * (Nr + 1); i++) {
        temp = rk[i - 1];
        if (i % Nk == 0) temp = SubWord(RotWord(temp)) ^ Rcon(i / Nk);
        else if (Nk > 6 && i % Nk == 4) temp = SubWord(temp);
        rk[i] = rk[i - Nk] ^ temp;
    }
    memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}

static void AddRoundKey(uint8_t* state, uint32_t* RoundKey) {
    uint32_t i;
    for (i = 0; i < Nb; i++) ((uint32_t*)state)[i] ^= RoundKey[i];
}

static void SubBytes(uint8_t* state) {
    uint32_t i;
    for (i = 0; i < AES_BLOCKLEN; i++) state[i] = sbox[state[i]];
}

static void ShiftRows(uint8_t* state) {
    uint8_t tmp;
    tmp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = tmp;
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    tmp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = tmp;
}

static uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1B : 0);
}

static void MixColumns(uint8_t* state) {
    uint8_t i, Tmp, Tm, t;
    for (i = 0; i < AES_BLOCKLEN; i += 4) {
        t = state[i];
        Tmp = state[i] ^ state[i+1] ^ state[i+2] ^ state[i+3];
        Tm = state[i] ^ state[i+1]; Tm = xtime(Tm); state[i] ^= Tm ^ Tmp;
        Tm = state[i+1] ^ state[i+2]; Tm = xtime(Tm); state[i+1] ^= Tm ^ Tmp;
        Tm = state[i+2] ^ state[i+3]; Tm = xtime(Tm); state[i+2] ^= Tm ^ Tmp;
        Tm = state[i+3] ^ t; Tm = xtime(Tm); state[i+3] ^= Tm ^ Tmp;
    }
}

static void InvSubBytes(uint8_t* state) {
    uint32_t i;
    for (i = 0; i < AES_BLOCKLEN; i++) state[i] = rsbox[state[i]];
}

static void InvShiftRows(uint8_t* state) {
    uint8_t tmp;
    tmp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = tmp;
    tmp = state[10]; state[10] = state[2]; state[2] = tmp;
    tmp = state[14]; state[14] = state[6]; state[6] = tmp;
    tmp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = tmp;
}

static void InvMixColumns(uint8_t* state) {
    uint8_t col;
    for (col = 0; col < AES_BLOCKLEN; col += 4) {
        uint8_t a = state[col], b = state[col+1], c = state[col+2], d = state[col+3];
        uint8_t e = xtime(a);
        uint8_t f = xtime(e);
        uint8_t g = xtime(f);
        uint8_t h = xtime(g);
        uint8_t i0 = h ^ g ^ e ^ b ^ c ^ d;
        uint8_t i1 = g ^ f ^ e ^ a ^ c ^ d;
        uint8_t i2 = f ^ e ^ a ^ b ^ d;
        uint8_t i3 = e ^ h ^ g ^ a ^ b ^ c;
        state[col] = i0 ^ xtime(g ^ f ^ b);
        state[col+1] = i1 ^ xtime(h ^ g ^ c);
        state[col+2] = i2 ^ xtime(g ^ e ^ d);
        state[col+3] = i3 ^ xtime(e ^ a);
    }
}

void AES_CBC_encrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, uint32_t length) {
    uint32_t i, j;
    uint8_t *Iv = ctx->Iv;
    for (i = 0; i < length; i += AES_BLOCKLEN) {
        for (j = 0; j < AES_BLOCKLEN; j++) buf[i+j] ^= Iv[j];
        AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey);
        for (j = 1; j < Nr; j++) {
            SubBytes(buf + i);
            ShiftRows(buf + i);
            MixColumns(buf + i);
            AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey + j * Nb);
        }
        SubBytes(buf + i);
        ShiftRows(buf + i);
        AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey + Nr * Nb);
        memcpy(Iv, buf + i, AES_BLOCKLEN);
    }
}

void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, uint32_t length) {
    uint32_t i, j;
    uint8_t storeNextIv[AES_BLOCKLEN];
    for (i = 0; i < length; i += AES_BLOCKLEN) {
        memcpy(storeNextIv, buf + i, AES_BLOCKLEN);
        AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey + Nr * Nb);
        for (j = Nr - 1; j > 0; j--) {
            InvShiftRows(buf + i);
            InvSubBytes(buf + i);
            AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey + j * Nb);
            InvMixColumns(buf + i);
        }
        InvShiftRows(buf + i);
        InvSubBytes(buf + i);
        AddRoundKey(buf + i, (uint32_t*)ctx->RoundKey);
        for (j = 0; j < AES_BLOCKLEN; j++) buf[i+j] ^= ctx->Iv[j];
        memcpy(ctx->Iv, storeNextIv, AES_BLOCKLEN);
    }
}